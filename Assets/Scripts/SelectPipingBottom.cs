...
public class SelectPipingBottom : MonoBehaviour
{
    ...
    void Start()
    {
        pipeAvailability = PlayerPrefs.GetString("PipeEdge");
    ...
    void pickPipe(Button b, int x)
    {
        ...
        PlayerPrefs.SetInt("PipeBottomTier" + tier, x);
    ...
    {
        for (int i = 0; i < transform.GetChild(shape).GetChild(0).GetChild(0).GetChild(2).GetChild(current).childCount; i++)
        {
            transform.GetChild(shape).GetChild(0).GetChild(0).GetChild(2).GetChild(current).GetChild(i).gameObject.SetActive(false);
        }
    }
    ...
    IEnumerator pipeAnimate(int current,int shape)
    {
        for (int i = 0; i < transform.GetChild(shape).GetChild(0).GetChild(0).GetChild(2).GetChild(current).childCount; i++)
        {
            transform.GetChild(shape).GetChild(0).GetChild(0).GetChild(2).GetChild(current).GetChild(i).gameObject.SetActive(true);
